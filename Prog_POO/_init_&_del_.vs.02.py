...
# Agora preciso colocar 3 comportamentos para o cachorro. Os métodos são bem parecidos com funções que estão dentro de uma classe.
    def latir (self):
        if self.acordado:
            ...
            self.acordado = False
            print('Zzzzz... O cachorro está dormindo.')
    def sentar(self,acordado=True):
	    print('Cachorro sentado.')
    def correr (self):
        if self.idade < 2 and self.acordado:
            print('Correndo rápido!!')
        if self.idade >= 2 and self.idade < 7:
            print('Correndo em velocidade média!!')
        if self.idade >= 7:
            print('Correndo devagarinho!!')            
        #print('Vapt Vupt!!')
...
estado = input("O dog está acordado? (s/n): ").strip().lower()
acordado = estado == "s"
